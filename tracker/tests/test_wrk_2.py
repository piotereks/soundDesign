...
def test_dur_calc(dummy_timeline):
    ...
    file_content = file_input_device.read(multi_track_file=True)
...
def test_note_at_beat():
    ...
    for tr in file_content:
        if tr.get(iso.EVENT_AMPLITUDE):
            tr[iso.EVENT_AMPLITUDE].sequence = [int(a*0.2) for a in tr[iso.EVENT_AMPLITUDE].sequence]

    track = next(f for f in file_content if f.get(iso.EVENT_NOTE))
    durs = list(track[iso.EVENT_DURATION])
    notes = list(track[iso.EVENT_NOTE])
